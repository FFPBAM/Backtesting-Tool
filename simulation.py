...
def build_returns(
    ...
    """
    needed = ["Equity", "Bonds", "Gold"]
    missing = [c for c in needed if c not in prices.columns]
    if missing:
        raise KeyError(
            f"Spalten {missing} fehlen in den Preisdaten. Vorhanden: "
            f"{list(prices.columns)}. Mögliche Ursache: veralteter "
            f"Streamlit-Cache — bitte 'Clear cache' in 'Manage app' "
            f"ausführen und die App neu starten."
        )
    rets = prices[needed].pct_change()
